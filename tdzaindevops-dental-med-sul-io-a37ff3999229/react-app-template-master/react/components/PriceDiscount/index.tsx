...
const PriceDiscount: FC<props> = ({ children }) => {
    ...
    return (
        <>
            <div>
                <p className='vtex-product-price-1-x-sellingPriceValue' style={{ margin: 0, display: 'flex' }}>{(value).toLocaleString('pt-BR', { minimumFractionDigits: 2, style: 'currency', currency: 'BRL' })}</p>
                {children}
                <p className='vtex-product-price-1-x-sellingPriceValue' style={{ fontSize: "15px", margin: '5px 0px', color: 'black', fontWeight: 'bold', display: 'flex', flexDirection: 'row', alignContent: 'center', justifyContent: 'center' }}>
                    {(value - (value * 0.03)).toLocaleString('pt-BR', { minimumFractionDigits: 2, style: 'currency', currency: 'BRL' })}
