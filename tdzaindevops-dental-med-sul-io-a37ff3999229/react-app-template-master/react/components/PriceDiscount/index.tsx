...
const PriceDiscount: FC<props> = ({children}) => {
    ...
    return (
        <>
            <div>
                <p className='vtex-product-price-1-x-sellingPriceValue' style={{ margin: 0 }}>{productContextValue?.product?.items[0].sellers[0].commertialOffer.Price.toLocaleString('pt-BR', { minimumFractionDigits: 2, style: 'currency', currency: 'BRL' })}</p>
                {children}
